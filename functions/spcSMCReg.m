...
% spcSMCReg register from SMC compressed data
...
addOptional(p, 'outputbinxy', 1); % Output image binxy
...
for iter = 1 : p.iterations
    ...
    for iframe = 1 : n
        ...
        if p.binxy > 1
            frame_cb = binxy(frame_c);
        end
        frame_cbln = localnormalizecore(frame_cb, p.hp_norm_sigmas, p.medfilt2size);
        frame_cblnf = frame_cbln(regfocus2(1) : regfocus2(2), regfocus2(3) : regfocus2(4));
        
        % Register
        [xy_shifts,~] = stackRegisterMA_RR(frame_cblnf, imref, 100, [], 0);
