...
spcpaths.smcreg = sprintf('%s_%s_%s%i_%s_smcreg.mat', date, mouse, RunOrSlice, run,...
    cstring);
spcpaths.smcdemreg = sprintf('%s_%s_%s%i_%s_smcdemreg.mat', date, mouse, RunOrSlice, run,...
    cstring);
...
% IEM99 output files
spcpaths.tif_iem99 = sprintf('%s_%s_%s%i_iem99.tif', date, mouse, RunOrSlice, ...
    run);
spcpaths.regtif_iem99 = sprintf('%s_%s_%s%i_iem99_reg.tif', date, mouse, RunOrSlice, ...
    run);
spcpaths.warptif_iem99 = sprintf('%s_%s_%s%i_iem99_warp.tif', date, mouse, RunOrSlice, ...
    run);
spcpaths.demregtif_iem99 = sprintf('%s_%s_%s%i_iem99_demreg.tif', date, mouse, RunOrSlice, ...
    run);
