...
if ~exist('dat','var')
    for dirID = 1:numel(slice_dir)
        currDir = slice_dir{dirID};
        currFiles = slices(strcmp(slices.folder,currDir),:);
        parfor f = 1:size(currFiles,1)
            photCount{dirID,f} = read_sdt(fullfile(currDir,currFiles.name{f}));
        end
    end
    for r = 1:size(photCount,1)
        dat(r).folder = fullfile(rootdir,slice_dir{r});
        dat(r).photCount = horzcat(photCount{r,:});
        dat(r).acc = sum(dat(r).photCount,2);
    end
end
...
%% Aggregate all data
allDat.photCount = horzcat(dat.photCount);
...
lower = [0,5,1,1,1.2,1.5,0];
upper = [0.2,max(bins),1e3,1e3,1.5,2,0];
